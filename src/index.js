        ...
        try {
            ...
            if (interaction.isStringSelectMenu() && interaction.customId === 'staff_application_select') {
                ...
                if (selectedValue === 'staff_application') {
                    ...
                    // --- تم التعديل حسب طلبك ---
                    const managementRoleId = '1419306155145953400'; // ID الرول الذي يمكنه رؤية التقديم
                    const applicationsCategoryId = '1397022482929549333'; // ID الكاتيجوري الخاص بتذاكر التقديم
                    // ---------------------------------------------
            ...
            if (interaction.isButton() && interaction.customId === 'ticket_claim') {
                // ... (كود استلام التذكرة - لا تغيير هنا)
            }

            // =================== الجزء المصحح لإغلاق جميع أنواع التذاكر ===================
            if (interaction.isButton() && interaction.customId === 'ticket_close') {
                const channel = interaction.channel;
                
                // محاولة العثور على فاتح التذكرة من أول رسالة في القناة
                let openerId = null;
                try {
                    const messages = await channel.messages.fetch({ limit: 1, after: 0 });
                    const firstMessage = messages.first();
                    // عادة أول رسالة تحتوي على منشن لفاتح التذكرة
